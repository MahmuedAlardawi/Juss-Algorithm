...
class PoemTokenizer:
                ...
                if re.match(self.tokens['letters_pattern'], cleaned_word[j]):
                    ...
                    if j < len(cleaned_word) - 1:
                        if re.match('ّ', cleaned_word[j + 1]):
                            if (j < len(cleaned_word) - 3 and re.match(self.tokens['madd_pattern'], cleaned_word[j + 2]) and
                                    re.match('ً', cleaned_word[j + 3])):
                            ...
                            if (i == len(poem_list) - 1 and j + 2 < len(cleaned_word) and re.match('ا', cleaned_word[j + 1]) and
                                    re.match('ً', cleaned_word[j + 2])):
                                letter_list.append(cleaned_word[j] + 'ً' + cleaned_word[j + 1])
                                j += 1

                            elif j + 2 < len(cleaned_word) and re.match(self.tokens['all_harakat_pattern'], cleaned_word[j + 2]):
                                letter_list.append(cleaned_word[j] + '' + cleaned_word[j + 1] + cleaned_word[j + 2])
                                j += 1
    ...
    def normalize_2(self, i, j):
        if 1 < j < 4 and self.is_al(i, j) and not re.match(self.tokens['harakat_pattern'], self.poem_list[i][j + 2]):
            return self.apply_al(i, j, 'none')
    ...
    def is_al(self, i, j):
        return (j + 3 < len(self.poem_list[i]) and re.match('ا', self.poem_list[i][j]) and re.match('ْ', self.poem_list[i][j + 1]) and
    ...
    def apply_special(self, word):
        if word == 'هذا':
            ...
            word = list('اَلْلَاه' + word[-1] if re.match(self.tokens['harakat_pattern'], word[-1]) else '')
        elif word == 'الرحمن':
            word.insert(9, 'ا')
        elif word == 'إله':
            word.insert(3, 'ا')
